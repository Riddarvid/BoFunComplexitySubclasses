...
-- >   n ( 0 0 ... 0 1 )
--
identity :: (Multiplicative a, Additive a) => Int -> Matrix a
identity n = matrix n n $ \(i,j) -> if i == j then A.one else A.zero
...
--
permMatrix :: (Multiplicative a, Additive a)
...
permMatrix n r1 r2 = matrix n n f
 where
  f (i,j)
   | i == r1 = if j == r2 then A.one else A.zero
   | i == r2 = if j == r1 then A.one else A.zero
   | i == j = A.one
   | otherwise = A.zero
...
inverse :: (Fractional a, Eq a, Multiplicative a, Additive a) => Matrix a -> Either String (Matrix a)
inverse m
    | ncols m /= nrows m
        = Left
...
combineRows :: (Additive a, Multiplicative a) => Int -> a -> Int -> Matrix a -> Matrix a
combineRows r1 l r2 m = mapRow (\j x -> x A.+ l A.* getElem r2 j m) r1 m
...
recLUDecomp u l p d k n =
 ...
 where
  ...
  d' = if i == k then d else A.negate d
...
--
--   'Nothing' is returned if no LU decomposition exists.
luDecomp' :: (Ord a, AddGroup a, MulGroup a) => Matrix a -> Maybe (Matrix a,Matrix a,Matrix a,Matrix a,a,a)
luDecomp' a = recLUDecomp' a i i (identity $ ncols a) A.one A.one 1 n
 where
  i = identity $ nrows a
  n = min (nrows a) (ncols a)

-- | Unsafe version of 'luDecomp''. It fails when the input matrix is singular.
luDecompUnsafe' :: (Ord a, AddGroup a, MulGroup a) => Matrix a -> (Matrix a, Matrix a, Matrix a, Matrix a, a, a)
luDecompUnsafe' m = case luDecomp' m of
  Just x -> x
  _      -> error "luDecompUnsafe' of singular matrix."

recLUDecomp' ::  (Ord a, AddGroup a, MulGroup a)
            =>  Matrix a -- ^ U
            ->  Matrix a -- ^ L
            ->  Matrix a -- ^ P
            ->  Matrix a -- ^ Q
...
recLUDecomp' u l p q d e k n =
 ...
 where
  ...
  (i, j) = maximumBy (comparing (\(i0, j0) -> aAbs $ u ! (i0,j0)))
  ...
  -- Permutation determinant
  d' = if i == k then d else A.negate d
  e' = if j == k then e else A.negate e
  -- Cancel elements below the pivot.
  (u'',l'') = go u' l' (k+1)
  ukk = u' ! (k,k)
  go u_ l_ h =
    if h > nrows u_
    then (u_,l_)
    else let x = (u_ ! (h,k)) A./ ukk
         in  go (combineRows h (A.negate x) k u_) (setElem x (h,k) l_) (h+1)
...
-- >          ( 4 5 6 )
-- > diagProd ( 7 8 9 ) = 45
diagProd :: Multiplicative a => Matrix a -> a
